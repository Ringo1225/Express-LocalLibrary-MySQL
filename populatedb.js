const sequelize = require("./models/sequelize"); // Import your Sequelize connection
...
async function bookCreate(index, title, summary, isbn, author, genre) {
  const bookdetail = {
    title: title,
    summary: summary,
    authorId: author.id, // Assuming authorId is the foreign key in the Book model
    isbn: isbn,
  };
...
async function bookInstanceCreate(index, book, imprint, due_back, status) {
  const bookinstancedetail = {
    bookId: book.id, // Assuming bookId is the foreign key in the BookInstance model
    imprint: imprint,
  };
...
// Run the main function to populate the MySQL database
